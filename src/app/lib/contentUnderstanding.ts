...
export class AzureContentUnderstandingClient {
  ...
  async beginAnalyze(
    ...
    if (
      ...
      throw new Error(
        "File path analysis not supported in this implementation. Use URL-based analysis."
      );
    }
  ...
  async pollResult(
    ...
    while (true) {
      ...
      console.log(
        `Waiting for service response (elapsed: ${(elapsed / 1000).toFixed(2)}s)`
      );

      if (elapsed > timeoutMs) {
        throw new Error(
          `Operation timed out after ${(elapsed / 1000).toFixed(2)} seconds.`
        );
      }
      ...
      if (status === "succeeded") {
        console.log(
          `Request result is ready after ${(elapsed / 1000).toFixed(2)} seconds.`
        );
        return result;
        ...
        const operationId =
          operationLocation.split("/").pop()?.split("?")[0] || "unknown";
  ...
  private transformToDocumentAnalysisResult(
    ...
    if (
      (contentResponse.status !== "Succeeded" &&
        contentResponse.status !== "succeeded") ||
          ...
          ([key, value]: [string, unknown]) => ({
            ...
            value:
              typeof value === "object" && value
                ? String(
                    (value as Record<string, unknown>).valueString ||
            ...
            confidence:
              typeof value === "object" && value
                ? ((value as Record<string, unknown>).confidence as number) || 0
                : 1,
          })
        )
      : [];
    ...
    const docTypeFromKVP = keyValuePairs.find(
      (kvp) => kvp.key.toLowerCase() === "doctype"
    )?.value;
...
export class ContentUnderstandingService {
  ...
  async analyzeDocumentComplete(
    ...
    try {
      ...
      const analysisResult = this.client["transformToDocumentAnalysisResult"](
        operationId,
        result
      );
      console.log(
  ...
  async analyzeDocument(
    ...
    try {
      ...
      if (!operationId) {
        throw new Error(
          "Could not extract operation ID from Operation-Location header"
        );
      }
  ...
  async pollForCompletion(
    ...
    const result = await this.client.pollResult(
      fakeResponse,
      timeoutSeconds,
  ...
  transformToDocumentAnalysisResult(
    ...
    return this.client["transformToDocumentAnalysisResult"](
      operationId,
      response as unknown as Record<string, unknown>
    );
  }

  async getDocumentFields(analyzerId: string): Promise<Record<string, any>> {
    const endpoint = `${process.env.FOUNDRY_ENDPOINT}/contentunderstanding/analyzers/${analyzerId}?${process.env.FOUNDRY_API_VERSION}`;
    const headers: Record<string, string> = {
      "Content-Type": "application/json",
      "Ocp-Apim-Subscription-Key": process.env.FOUNDRY_KEY || "",
    };
    const response = await fetch(endpoint, { method: "GET", headers });
    if (!response.ok) {
      throw new Error(`Failed to fetch analyzer schema: ${response.status}`);
    }

    const data = await response.json();
    console.log(data);
    return data.fieldSchema?.fields || {};
  }

  // Analyzer Management Functions

  /**
   * List all analyzers
   */
  async listAnalyzers(): Promise<any[]> {
    const endpoint = `${process.env.FOUNDRY_ENDPOINT}/contentunderstanding/analyzers?api-version=${process.env.FOUNDRY_API_VERSION}`;
    const headers: Record<string, string> = {
      "Content-Type": "application/json",
      "Ocp-Apim-Subscription-Key": process.env.FOUNDRY_KEY || "",
    };
    ...
    const response = await fetch(endpoint, { method: "GET", headers });
    if (!response.ok) {
      throw new Error(`Failed to list analyzers: ${response.status}`);
    }
   ...
   */
  async getAnalyzer(analyzerId: string): Promise<any> {
    const endpoint = `${process.env.FOUNDRY_ENDPOINT}/contentunderstanding/analyzers/${analyzerId}?api-version=${process.env.FOUNDRY_API_VERSION}`;
    const headers: Record<string, string> = {
      "Content-Type": "application/json",
      "Ocp-Apim-Subscription-Key": process.env.FOUNDRY_KEY || "",
    };
    ...
    const response = await fetch(endpoint, { method: "GET", headers });
    if (!response.ok) {
      throw new Error(`Failed to get analyzer: ${response.status}`);
    }
  ...
  /**
   * Create or replace analyzer (standard)
   */
  async createOrReplaceAnalyzer(
    analyzerId: string,
    analyzerData: any
  ): Promise<any> {
    const endpoint = `${process.env.FOUNDRY_ENDPOINT}/contentunderstanding/analyzers/${analyzerId}?api-version=${process.env.FOUNDRY_API_VERSION}`;
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to create/replace analyzer: ${response.status} - ${errorText}`
      );
    }
   ...
   * Update analyzer
   */
  async updateAnalyzer(analyzerId: string, updateData: any): Promise<any> {
    const endpoint = `${process.env.FOUNDRY_ENDPOINT}/contentunderstanding/analyzers/${analyzerId}?api-version=${process.env.FOUNDRY_API_VERSION}`;
    const headers: Record<string, string> = {
      "Content-Type": "application/json",
      "Ocp-Apim-Subscription-Key": process.env.FOUNDRY_KEY || "",
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to update analyzer: ${response.status} - ${errorText}`
      );
    }
  ...
  /**
   * Replace analyzer (delete and recreate)
   */
  async replaceAnalyzer(
    analyzerId: string,
    analyzerData: any
  ): Promise<any> {
    try {
      // First, delete the existing analyzer
      await this.deleteAnalyzer(analyzerId);
      
      // Wait a short time to ensure deletion is complete
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // Then create the new analyzer
      return await this.createOrReplaceAnalyzer(analyzerId, analyzerData);
    } catch (error) {
      throw new Error(`Failed to replace analyzer: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  async getAnalyzerOperationStatus(
    analyzerId: string,
    operationId: string
  ): Promise<any> {
    const endpoint = `${process.env.FOUNDRY_ENDPOINT}/contentunderstanding/analyzers/${analyzerId}/operations/${operationId}?api-version=${process.env.FOUNDRY_API_VERSION}`;
    const headers: Record<string, string> = {
      "Content-Type": "application/json",
      "Ocp-Apim-Subscription-Key": process.env.FOUNDRY_KEY || "",
    };
